...
export default function ClientSidePDFAnalyzer() {
  ...
  useEffect(() => {
    console.log('🤖 Initializing CLIENT-SIDE PDF.js with Next.js 15...');
    setPdfjs(PdfJs);
  }, []);
  ...
  const analyzePDF = async () => {
    ...
    try {
      console.log('🚀 Starting CLIENT-SIDE PDF analysis with Next.js 15...');
          ...
          annotations.forEach((annotation, index) => {
            ...
            if (annotation.subtype === 'Widget' || annotation.fieldName) {
              ...
              allFields.push({
                ...
                method: 'Client-Side PDF.js Annotations',
                confidence: 1.0,
                subtype: annotation.subtype,
                fontSize: 10
              });
      ...
      const result = {
        ...
        fields: allFields,
        method: 'Client-Side PDF.js Analysis (Next.js 15)',
        note: 'Using client-side PDF.js with webpack.mjs - NO SERVER CALLS!'
      };
      ...
      console.error('❌ Client-side PDF analysis error:', err);
      ...
      fieldPatterns.forEach(pattern => {
        if (pattern.pattern.test(text)) {
          ...
          fields.push({
            ...
            method: 'Client-Side Text Pattern Analysis',
            confidence: 0.8,
            sourceText: item.str,
            fontSize: 10
          });
  ...
  const saveDetectedSchema = async (result, pdfFileName) => {
    try {
      ...
      const fileName = `${baseName}_client_side_detected_schema.json`;
      
      // Create schema object
      const schema = {
        formType: "client_side_detected",
        name: `${pdfFileName} - Client-Side Detected Fields`,
        description: `Automatically detected form fields from ${pdfFileName} using client-side PDF.js`,
        version: "1.0",
      ...
      console.log(`💾 Saved client-side detected schema: ${fileName}`);
    } catch (err) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-lg">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-800 mb-2">🚀 Client-Side PDF Field Detector</h1>
        <p className="text-gray-600">
          Next.js 15 + PDF.js client-side analysis - NO server calls, NO webpack issues!
        </p>
      </div>
                ...
                {analysisResult.fields.map((field, index) => (
                  <div key={index} className="flex justify-between items-center p-2 bg-gray-50 rounded text-sm">
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-xs font-medium text-purple-600">{field.type}</div>
                    </div>
                  </div>
                ))}
              </div>
